...
export default function VIPReservationPage() {
  ...
  const eventType = searchParams.get("type") || "weddings"; // default to weddings
  ...
  const eventDetails = {
    weddings: {
      ...
      serviceOptions: [
        ...
        { name: "Guest Management & Concierge", price: 2000 },
      ],
    },
    birthday: {
      name: "Luxury Birthday Parties",
      icon: <Cake className="w-12 h-12 mx-auto mb-4 text-yellow-500" />,
      serviceOptions: [
    ...
    graduation: {
      name: "Luxury Graduations",
      icon: <GraduationCap className="w-12 h-12 mx-auto mb-4 text-green-500" />,
      serviceOptions: [
        { name: "Luxury Hall", price: 7000 },
        { name: "Professional Photography & Videography", price: 5000 },
        { name: "Catering", price: 400 }, // per guest
        { name: "Luxury Decoration", price: 4000 },
        { name: "DJ & Entertainment", price: 3000 },
        { name: "VIP Car Service", price: 2000 },
        { name: "Guest Management", price: 1500 },
      ],
    },
    generalevents: {
      name: "Luxury Corporate & General Events",
  ...
  const serviceOptions = eventDetails[eventType]?.serviceOptions || [];
  ...
  }, [formData.selectedServices, formData.guests, serviceOptions]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/vip/${eventType}`, {
        method: "POST",
        body: form,
      });
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      <header className="bg-gradient-to-r from-purple-600 to-pink-500 text-white py-16 text-center px-4">
        {eventDetails[eventType]?.icon}
        <h1 className="text-4xl font-bold">{eventDetails[eventType]?.name}</h1>
